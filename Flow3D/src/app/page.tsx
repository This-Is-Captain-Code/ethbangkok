...
const Page: React.FC = ()  => {
  ...
  // Mint NFT using Web3.js
  const mintNFT = async (uid: string) => {
    if (!ethAddress) {
      alert("Please connect your MetaMask wallet to mint an NFT.");
      return;
    }
    ...
    try {
      ...
      const contractAddress = "0x074ddb3000b6865b4a6dBF9c06134F171106F305"; // Your contract address
      const abi = [
        {
          "inputs": [
            { "internalType": "string", "name": "downloadLink", "type": "string" },
      ...
      const tx = await contract.methods.mint(metadata.download_link, metadata.image, metadata.name)
